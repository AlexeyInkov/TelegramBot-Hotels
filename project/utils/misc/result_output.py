...
def get_result(message: Message, dict_set: dict) -> None:
	...
	for elem in response2['data']['propertySearch']['properties']:
		...
		answer = f'id Отеля: {id_hotel}\nОтель: {name_hotel}\n' \
					f'Адрес отеля: {address_hotel}\n' \
					f'Расстояние до центра: {far_centr}км\n' \
					f'Стоимость за ночь(но не точно): ${cost}\n'
		bot.send_message(message.from_user.id, answer)
		logger.debug('Запрос от {} {} в городе {}'.format(message.from_user.first_name,
